...
function Push-Repository {
  ...
  $pushArgs = @('-C', $RepoPath, '-c', 'http.sslBackend=schannel', 'push', '-u', $Remote, "HEAD:$PushBranch")
  $fallbackArgs = @('-C', $RepoPath, 'push', '-u', $Remote, "HEAD:$PushBranch")
  if ($ReplaceRemoteHistory) {
    $pushArgs += '--force-with-lease'
    $fallbackArgs += '--force-with-lease'
  }

  $forceArgs = @('-C', $RepoPath, 'push', '--force', '-u', $Remote, "HEAD:$PushBranch")

  try {
    Invoke-Git -Arguments $pushArgs | Out-Null
  } catch {
    try {
      Invoke-Git -Arguments $fallbackArgs | Out-Null
    } catch {
      if (-not $ReplaceRemoteHistory) {
        throw
      }

      Invoke-Git -Arguments $forceArgs | Out-Null
    }
  }
}
...
if ($needsCleanRepo -and -not $PublishDir) {
  ...
  $PublishDir = Join-Path (Split-Path $resolvedSource -Parent) "$folderName-publish"
}
